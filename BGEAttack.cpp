...
int BGEAttack::proposition2(mat_GF2 & L, NTL::mat_GF2 & out, mat_GF2 beta){
	...
	cout << "My system of equations looks like: " << endl;
	dumpMatrix(eqSystem);

	int rank = gauss(eqSystem);
	cout << "Gauss form; rank="<<rank<<"; matrix: " << endl;
	dumpMatrix(eqSystem);
	...
	if (determinant == GF2::zero()){
		cout << "Problem with solving system of equations in proposition2, determinant=0; rank=" << rank << endl;
		cout << "Input L matrix: " << endl; dumpMatrix(L);
		cout << "Beta matrix: " << endl; dumpMatrix(beta);

		// TODO: return set of orthogonal vectors that solve this homogenous equation in case of singular system of
	...
	cout << "My solution looks like: " << endl;
	dumpMatrix(out);

	if (trivialSolution){
		cout << "We obtained trivial solution...";
		return -3;
	}

	// Final verification of given solution - is equation on the top correct now?
	mat_GF2 lhs = L * out;
	mat_GF2 rhs = out * beta;
	if(lhs != rhs){
...
int BGEAttack::run(void) {
	...
	if (charPolyMap.count(LpolyInt)==0){
		...
		//for(auto it = bset2.begin(); it!=bset2.end(); ++it){
			...
			mat_GF2 Atild;
			int AtildResult = proposition2(L, Atild, defAES.makeMultAMatrix(possBeta));
			if (AtildResult != 0){
